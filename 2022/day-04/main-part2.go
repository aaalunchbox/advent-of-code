...
// Check if assignments partially overlap
func partialOverlap(a [][]int) bool {
	set := make(map[int]int)

	// Check for full overlap, pretty quick
	if fullOverlap(a) {
		return true
	}
	
	// Create a set of values in assignment 1
	for i := a[0][0]; i <= a[0][1]; i++ {
		set[i] = i
	}

	// Test if assignment 2 is in the set
