...
func main() {
	...
	for scanner.Scan() {
		...
		for i := 0; i < move[0]; i++ {
			crates = moveCrate(crates, move[1] - 1, move[2] - 1)
		}
	}
	fmt.Println(crates)
	var answer string
...
func moveCrate(crates [][]string, from int, to int) [][]string {
	...
	l := len(crates[from])
	c := crates[from][l - 1]  // Get Crate
	crates[from] = crates[from][:l - 1]  // Remove crate from stack
